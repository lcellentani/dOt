...
Teapot::Teapot(int grid, const mat4 & lidTransform)
{
	...
	unsigned int * el = new unsigned int[mFaces * 6];
...
void Teapot::BuildPatchReflect(int patchNum, float *B, float *dB, float *v, float *n, float *tc, unsigned int *el, int &index, int &elIndex, int &tcIndex, int grid, bool reflectX, bool reflectY)
{
	...
	if (reflectX)
	{
		BuildPatch(patchRevV, B, dB, v, n, tc, el, index, elIndex, tcIndex, grid, mat3(vec3(-1.0f, 0.0f, 0.0f), vec3(0.0f, 1.0f, 0.0f), vec3(0.0f, 0.0f, 1.0f)), false);
	}
	...
	{
		BuildPatch(patch, B, dB, v, n, tc, el, index, elIndex, tcIndex, grid, mat3(vec3(-1.0f, 0.0f, 0.0f), vec3(0.0f, -1.0f, 0.0f), vec3(0.0f, 0.0f, 1.0f)), true);
	}
}
...
void Teapot::GetPatch(int patchNum, vec3 patch[][4], bool reverseV)
{
	for (int u = 0; u < 4; u++) // Loop in u direction
	{
		for (int v = 0; v < 4; v++) // Loop in v direction
		{
			if (reverseV) {
				patch[u][v] = vec3(
					TeapotModel::cpdata[TeapotModel::patchdata[patchNum][u * 4 + (3 - v)]][0],
					TeapotModel::cpdata[TeapotModel::patchdata[patchNum][u * 4 + (3 - v)]][1],
					TeapotModel::cpdata[TeapotModel::patchdata[patchNum][u * 4 + (3 - v)]][2]
				);
			}
			else
			{
				patch[u][v] = vec3(
					TeapotModel::cpdata[TeapotModel::patchdata[patchNum][u * 4 + v]][0],
					TeapotModel::cpdata[TeapotModel::patchdata[patchNum][u * 4 + v]][1],
					TeapotModel::cpdata[TeapotModel::patchdata[patchNum][u * 4 + v]][2]
				);
			}
		}
	}
